...
static esp_err_t capture_handler(httpd_req_t *req)
{
	...
	#if MICROPY_HW_USB_CAM
	if(stream_type){
		snprintf(ts, 32, "%lld.%06ld", uvc_fb->timestamp.tv_sec, uvc_fb->timestamp.tv_usec);
	}else
	#endif
	{
	#if MICROPY_ENABLE_SENSOR
		snprintf(ts, 32, "%lld.%06ld", ov2640_fb->timestamp.tv_sec, ov2640_fb->timestamp.tv_usec);
	#endif
	}
...
static esp_err_t stream_handler(httpd_req_t *req)
{
	...
	while (true)
	{
		...
		#endif
		if(res == ESP_ERR_HTTPD_RESP_SEND || !is_streaming){
