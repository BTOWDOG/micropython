...
STATIC void gui_btn_init(void){
	if(!is_init){
		for(uint16_t i=0; i < GUI_BTN_NUM_MAX; i++){
			btn_obj_all[i] = heap_caps_malloc(sizeof(gui_button_obj_t),MALLOC_CAP_8BIT);
			gui_button_obj_t *self = btn_obj_all[i];	
			if(self == NULL){
				mp_raise_ValueError(MP_ERROR_TEXT("gui_btn_init malloc error"));
			}
			self->is_enabled = 0;
			self->gui_btn = (_btn_obj*)heap_caps_malloc(sizeof(_btn_obj),MALLOC_CAP_8BIT);
			if(self->gui_btn == NULL){
				mp_raise_ValueError(MP_ERROR_TEXT("gui_btn_init malloc error"));
			}
			self->gui_btn->label =(char *)heap_caps_malloc(GUI_BTN_STR_LEN,MALLOC_CAP_8BIT);
		}
		is_init = 1;
	}
}
...
MP_DEFINE_CONST_OBJ_TYPE(
	...
	// print, gui_button_print,
	make_new, gui_button_make_new,
	locals_dict, &gui_button_locals_dict
);
MP_REGISTER_ROOT_POINTER(struct _gui_button_obj_t *gui_btn_obj_all[GUI_BTN_NUM_MAX]);
