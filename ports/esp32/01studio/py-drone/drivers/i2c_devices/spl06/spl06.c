...
void spl0601_get_calib_param(void)
{
	...
	#if MICROPY_HW_I2CDEV_V1
	spl06i2cdevRead(I2Cx, SPL06_COEFFICIENT_CALIB_REG, SPL06_CALIB_COEFFICIENT_LENGTH, buffer);
	#else
...
void spl0601_rateset(spl06Sensor_e sensor, uint8_t measureRate, uint8_t oversamplRate)
{
	...
	if (sensor == PRESURE_SENSOR)
	{
		...
		#if MICROPY_HW_I2CDEV_V1
		i2cdevWriteByte(I2Cx->devHandle[SPL06], SPL06_PRESSURE_CFG_REG, measureRate<<4 |oversamplRate);
		#else
		...
		{
			#if MICROPY_HW_I2CDEV_V1
			i2cdevReadByte(I2Cx->devHandle[SPL06], SPL06_INT_FIFO_CFG_REG, &reg);
			i2cdevWriteByte(I2Cx->devHandle[SPL06], SPL06_INT_FIFO_CFG_REG, reg | 0x04);
			#else
	...
	else if (sensor == TEMPERATURE_SENSOR)
	{
		...
		#if MICROPY_HW_I2CDEV_V1
		i2cdevWriteByte(I2Cx->devHandle[SPL06], SPL06_TEMPERATURE_CFG_REG, measureRate<<4 | oversamplRate | 0x80);
		#else
		...
		if (oversamplRate > SPL06_OVERSAMP_8)
		{
			#if MICROPY_HW_I2CDEV_V1
			i2cdevReadByte(I2Cx->devHandle[SPL06], SPL06_INT_FIFO_CFG_REG, &reg);
			i2cdevWriteByte(I2Cx->devHandle[SPL06], SPL06_INT_FIFO_CFG_REG, reg | 0x08);
			#else
			i2cdevReadByte(I2Cx, devAddr, SPL06_INT_FIFO_CFG_REG, &reg);
...
bool SPL06Init(I2C_Dev *i2cPort)
{
	...
	i2c_device_config_t conf = {
		...
		.device_address  = SPL06_I2C_ADDR,
		.scl_speed_hz	 = 400000,
	};

	i2c_master_bus_add_device(I2Cx->busHandle, &conf, &I2Cx->devHandle[SPL06]);
	
	vTaskDelay(1 / portTICK_RATE_MS);

	i2cdevReadByte(I2Cx->devHandle[SPL06], SPL06_CHIP_ID, &SPL06ID);	/* 读取SPL06 ID*/
	...
	spl0601_rateset(PRESURE_SENSOR, SPL06_MWASURE_16, SPL06_OVERSAMP_32);
	spl0601_rateset(TEMPERATURE_SENSOR, SPL06_MWASURE_16, SPL06_OVERSAMP_1);
	
	i2cdevWriteByte(I2Cx->devHandle[SPL06], SPL06_MODE_CFG_REG, SPL06_MODE);
