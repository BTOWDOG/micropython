                    ...
                    uint8_t bitStart, uint8_t length, uint8_t *data)
{
    ...
    if((status = i2cdevReadByte(devHandle, memAddress, &byte)) == true){
...
{
    return i2c_master_transmit_receive(devHandle, &memAddress, 1, data, len, -1);
}
                     ...
                     uint8_t bitStart, uint8_t length, uint8_t data)
{
    ...
    if((status = i2cdevReadByte(devHandle, memAddres, &byte)) == true){
        uint8_t mask = ((1 <<length) - 1) << (bitStart - length + 1);
                     ...
                     uint8_t len, uint8_t *data)
{
    ...
    return i2c_master_transmit(devHandle, buf, len + 1, 500 / portTICK_PERIOD_MS);
}
