...
{	
	...
	*thrust = constrainf(thrustRaw + configParam.thrustBase, 1000, 60000);	/*油门限幅*/
	...
	// printf("pos_sp %f pos %f vel_sp %f\n",
	// 		setpoint->position.z,
	// 		state->position.z,
	// 		setpoint->velocity.z);
	...
	float errVz = setpoint->velocity.z - state->velocity.z;
	thrustRaw = pidUpdate(&pidVZ, errVz);

	// DBG_EVERY(pid_z_log, 100,
	// 	"PIDZ modeZ=%d spPosZ=%.2f posZ=%.2f spVelZ=%.2f velZ=%.2f errVz=%.2f pidZout=%.2f pidVZout=%.2f thrust=%0.1f thrustLpf=%.1f cfgBase=%d",
	// 	setpoint->mode.z,
	// 	setpoint->position.z,
	// 	state->position.z,
	// 	setpoint->velocity.z,
	...
	// 	pidZ.out,
	// 	pidVZ.out,
	// 	thrustRaw + THRUST_BASE,
	// 	thrustLpf,
	// 	configParam.thrustBase);
}
