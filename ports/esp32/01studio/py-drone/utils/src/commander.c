...
{	
	...
	if(commander.ctrlMode & 0x01)/*定高模式*/
	{
		...
		{	
			...
			// DBG_EVERY(cmd_z_log, 50,
			// "CMD ctrlMode=%u keyFlight=%d keyLand=%d thrustIn=%d climb=%.1f modeZ=%d spPosZ=%.2f spVelZ=%.2f statePosZ=%.2f errPosZ=%.2f",
			// getCommanderCtrlMode(),
			// commander.keyFlight,
			// commander.keyLand,
			// ctrlValLpf.thrust,
			// climb,
