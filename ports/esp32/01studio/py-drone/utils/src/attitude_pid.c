...
void attitudeRatePID(Axis3f *actualRate,attitude_t *desiredRate,control_t *output)	/* 角速度环PID */
{
    ...
    // if (++tick % 500 == 0) {
    //     printf("RATE sp_rr=%.2f act_rr=%.2f err_rr=%.2f out_r=%d | sp_pr=%.2f act_pr=%.2f err_pr=%.2f out_p=%d\n",
    //            desiredRate->roll, actualRate->x, desiredRate->roll - actualRate->x, output->roll,
    //            desiredRate->pitch, actualRate->y, desiredRate->pitch - actualRate->y, output->pitch);
    // }
}
...
void attitudeAnglePID(attitude_t *actualAngle,attitude_t *desiredAngle,attitude_t *outDesiredRate)	/* 角度环PID */
{
    ...
    static uint32_t tick = 0;
    // if (++tick % 500 == 0) {
    //     printf("ANG sp_r=%.2f act_r=%.2f err_r=%.2f out_r=%.2f | sp_p=%.2f act_p=%.2f err_p=%.2f out_p=%.2f\n",
    //            desiredAngle->roll, actualAngle->roll, desiredAngle->roll - actualAngle->roll, outDesiredRate->roll,
    //            desiredAngle->pitch, actualAngle->pitch, desiredAngle->pitch - actualAngle->pitch, outDesiredRate->pitch);
    // }
