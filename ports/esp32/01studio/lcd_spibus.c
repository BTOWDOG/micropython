...
void  lcd_bus_init(void)
{
	if(!is_init)
	{
		...
		gpio_reset_pin(self->dc);
		#if !MICROPY_ENABLE_PCA9557
		gpio_reset_pin(self->rst);
		#endif
...
void lcd_spibus_deinit(void) {
	if(is_init){
		#if !MICROPY_ENABLE_PCA9557
		gpio_set_level(lcd_spibus->rst, 0);
		vTaskDelay(10 / portTICK_PERIOD_MS);
		gpio_set_level(lcd_spibus->rst, 1);
		vTaskDelay(10 / portTICK_PERIOD_MS);
		#else
		if(lcd_spibus!= NULL) 
		{
			for (int i = 0; i < 5; i++) {
				...
				lcd_spibus_send_cmd(lcd_spibus, 0x00);
				vTaskDelay(10 / portTICK_PERIOD_MS);
			}
			vTaskDelay(10 / portTICK_PERIOD_MS);
			lcd_spibus_send_cmd(lcd_spibus, 0x01);
			vTaskDelay(150 / portTICK_PERIOD_MS);
			lcd_spibus_send_cmd(lcd_spibus, 0x11);
