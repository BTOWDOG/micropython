    ...
    while (true)
    {
        if (gEvent)
        {
            if (xQueueReceive(xQueueFrameI, &(frame1), portMAX_DELAY))
            {
                ...
                {
                    uint32_t moving_point_number = dl::image::get_moving_point_number((unsigned short*)frame1->buf, (unsigned short*)frame2->buf, (unsigned int)frame1->height, (unsigned int)frame1->width, 8, 15);
					#if MICROPY_HW_ESPAI
