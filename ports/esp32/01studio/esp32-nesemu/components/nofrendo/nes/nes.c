...
static void nes_renderframe(bool draw_flag)
{
   ...
   // printf("draw_flag %d scanlin%d\n",draw_flag,nes.scanline);
   // printf("bmp=%p  line0=%p\n", bmp, bmp->line[0]);
...
//      ppu_scanline(nes.vidbuf, nes.scanline, draw_flag);
   ...
   // uint16 *p = (uint16*)bmp->line[0];
   //  printf("scan100: px0=%04x px10=%04x\n", p[0], p[10]);
   nes.scanline = 0;
}
...
void nes_emulate(void)
{
   ...
   while (false == nes.poweroff)
   {
      if (nofrendo_ticks != last_ticks)
      {
         ...
         // printf("nofrendo_ticks =%d  last_ticks=%d frames_to_rende = %d\n",nofrendo_ticks,last_ticks,frames_to_render);
         gui_tick(tick_diff);
         last_ticks = nofrendo_ticks;
      }
