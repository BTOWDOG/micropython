...
static float InvSqrt(float x)
{
	...
	// x = x*(1.5f-xhalf*x*x);
	// return x;
    // 定义一个union，将int和float共享同一内存
    union {
        float f;
        int i;
    } conv;

    conv.f = x;  // 将float赋值给union中的f
    conv.i = 0x5f3759df - (conv.i >> 1);  // 执行位操作
    x = conv.f;  // 从union中提取回float

    // Newton-Raphson迭代，进一步优化结果
    float xhalf = 0.5f * x;
    x = x * (1.5f - xhalf * x * x);
    return x;
}
